  ...
  const handleCalculate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/scheme/${code}/rolling-returns?window=${window}y&duration=${duration}y&interval=${interval}`
      );
      ...
      <div className="xl:col-span-2">
        <div className="bg-white rounded-3xl shadow-lg p-8">
          ...
          <form onSubmit={handleCalculate} className="space-y-4">
            ...
            <div>
              <label className="text-sm font-semibold text-light-text">Data Interval</label>
              <select
                value={interval}
          ...
          {result && result.averageReturn !== undefined && (
            <div className="w-full">
              ...
              <p className="text-sm text-light-text text-center">
                Based on {result.count} rolling {window}-year periods over the last {duration} years.
              </p>
