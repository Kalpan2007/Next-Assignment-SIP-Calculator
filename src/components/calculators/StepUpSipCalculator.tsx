        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
            <div className="xl:col-span-2">
                <div className="bg-white rounded-3xl shadow-lg p-8">
                    <h3 className="text-2xl font-bold mb-6">Step-Up SIP Calculator</h3>
                    <form onSubmit={handleCalculate} className="space-y-4">
                        <div>
                            <label className="text-sm font-semibold text-light-text">Initial Monthly Amount</label>
                            <input type="number" value={initialAmount} onChange={e => setInitialAmount(e.target.value)} className="w-full p-2 border-2 rounded-lg mt-1" min="1" max="100000000"/>
                        </div>
                        <div>
                            <label className="text-sm font-semibold text-light-text">Annual Increase (%)</label>
                            <input type="number" value={annualIncreasePercent} onChange={e => setAnnualIncreasePercent(e.target.value)} className="w-full p-2 border-2 rounded-lg mt-1" min="0" max="100"/>
                        </div>
            ...
            <div className="xl:col-span-3">
                ...
                {result && (
                     <div className="bg-white p-8 rounded-xl shadow-lg grid grid-cols-2 gap-6 text-center">
                        <div><p className="text-sm text-light-text">Total Invested</p><p className="text-2xl font-bold">{formatCurrency(result.totalInvested)}</p></div>
                        <div><p className="text-sm text-light-text">Current Value</p><p className="text-2xl font-bold">{formatCurrency(result.currentValue)}</p></div>
                        <div><p className="text-sm text-light-text">Absolute Return</p><p className="text-2xl font-bold text-green-600">{result.absoluteReturn.toFixed(2)}%</p></div>
                        <div><p className="text-sm text-light-text">Annualized Return</p><p className="text-2xl font-bold text-brand-orange">{result.annualizedReturn ? `${result.annualizedReturn.toFixed(2)}%` : 'N/A'}</p></div>
                    </div>
                )}
            </div>
        </div>
